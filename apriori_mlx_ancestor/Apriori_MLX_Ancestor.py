"""
Association-rule mining pipeline — mlxtend Apriori with ancestor expansion.

Usage:
    python Apriori_MLX_Ancestor.py <base> [options]
...
def parse_args():
    ...
    p.add_argument("--mode", choices=["flat", "basic"], default="flat",
                   help="flat uses one taxonomy level; basic expands each "
                        "category path with its ancestor prefixes.")
    p.add_argument("--k-levels", type=int, default=None, metavar="K",
                   help="For --mode basic, only use the last K taxonomy levels. "
                        "If omitted, the full category path is expanded.")
    ...
    p.add_argument("--low-memory", action="store_true",
                   help="Use mlxtend Apriori's low_memory mode. This is slower, "
                        "but avoids large sparse candidate slices for dense "
...
def _expand_path_with_ancestors(category_name: str, k_levels: Optional[int] = None) -> List[str]:
    parts = [p.strip() for p in str(category_name).split(">") if p.strip()]
    if k_levels is not None:
        if k_levels <= 0:
            return []
        parts = parts[-k_levels:]
...
def build_basic_transactions(
    df: pd.DataFrame,
    k_levels: Optional[int] = None,
) -> Tuple[List[List[str]], int]:
    df = df.copy()
    df["item"] = df["category_name"].apply(
        lambda s: _expand_path_with_ancestors(s, k_levels)
    )
    df = df.explode("item").dropna(subset=["item"])
    df = df.drop_duplicates(subset=["wishlist_id", "item"])
...
def main():
    ...
    with timed_step("Building transactions"):
        if args.mode == "basic":
            baskets, _ = build_basic_transactions(df, args.k_levels)
        else:
